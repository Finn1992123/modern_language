...
begin
  ...
  if input_achievement_type not in ('try', 'dictation', 'participation')
    and recipient_count <> 1 then
    raise exception 'This achievement can only be awarded to one student'
      using errcode = '22023';
  end if;
...
create or replace function public.get_teacher_junior_achievement_leaderboard(
  input_class_id uuid,
  input_achievement_type text
)
returns table (
...
grant execute on function public.get_teacher_junior_achievement_leaderboard(
  uuid,
  text
) to authenticated;
