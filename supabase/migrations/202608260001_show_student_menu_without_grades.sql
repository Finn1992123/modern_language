...
as $$
  ...
  join public.classes c
    on c.id = sl.class_id
  left join public.students_grades sg
    on sg.student_language_id = sl.id
  ...
  where sl.student_id = input_student_id
    and exists (
      select 1
  ...
  group by c.id, c.name, c.language, c.days_hours
  order by c.language;
$$;

grant execute on function public.get_student_language_averages(uuid)
to authenticated;
