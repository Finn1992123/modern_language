...
class _TeacherClassesPageState extends State<TeacherClassesPage> {
  ...
  final Set<String> _completedClassIds = {};
  ...
  Future<void> _loadClasses() async {
    ...
    try {
      ...
      final completionRows = await Supabase.instance.client.rpc(
        'get_teacher_class_completion_today',
        params: {'input_local_date': _formatIsoDate(DateTime.now())},
      );
      final completedClassIds = _readCompletedClassIds(completionRows);
  ...
  Future<void> _selectClass(_TeacherClass teacherClass) async {
    ...
    try {
      ...
      setState(() {
        ...
        _updateSelectedClassCompletion();
      });
    } catch (_) {
      if (!mounted) {
        return;
  ...
  void _updateSelectedClassCompletion() {
    final selectedClass = _selectedClass;
    if (selectedClass == null) {
      return;
    }

    final hasCurriculum = _completedCurriculumClassIds.contains(
      selectedClass.id,
    );
    final allStudentsCompleted =
        _students.isNotEmpty &&
        _students.every(
          (student) =>
              _studentForms[student.studentLanguageId]?.completionMessage !=
              null,
        );

    if (hasCurriculum && allStudentsCompleted) {
      _completedClassIds.add(selectedClass.id);
    } else {
      _completedClassIds.remove(selectedClass.id);
    }
  }
      ...
      children: [
        for (final teacherClass in _classes) ...[
          _ClassTile(
            ...
            isCompleted: _completedClassIds.contains(teacherClass.id),
        ...
        children: [
          Text(
            ...
            style: const TextStyle(
              color: _TeacherClassesPageState._contentColor,
              fontSize: 21,
              fontWeight: FontWeight.w900,
            ),
          ),
          ...
          child: Row(
            children: [
              ...
              Icon(
                ...
                color: isCompleted ? const Color(0xFF58B832) : null,
              ),
            ],
          ),
        ),
...
Set<String> _readCompletedClassIds(Object? rows) {
  if (rows is! List) {
    return <String>{};
  }
...
class _InfoPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _SectionCard(
      child: Row(
        children: [
          Icon(icon, color: _TeacherClassesPageState._contentColor),
          const SizedBox(width: 12),
          Expanded(
            child: Text(
              text,
